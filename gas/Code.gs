...
function doGet(e) {
  ...
  if (action === "verify_password") {
    ...
    if (!actualPassword) actualPassword = "10101010"; // 預設密碼
  ...
  try {
    ...
    if (action === "update_github_file") {
      ...
      if (!fileName || !htmlContent) {
        return jsonResponse({ success: false, error: "Missing fileName or html" });
      }

      // 1. 先抓取 GitHub 倉庫中該檔案的 SHA 雜湊
      ...
      if (getResp.getResponseCode() !== 200) {
        return jsonResponse({ success: false, error: "無法在 GitHub 找到此檔案: " + getResp.getContentText() });
      }

      const fileData = JSON.parse(getResp.getContentText());
      const fileSha = fileData.sha;

      // 2. 將 HTML 轉為 Base64 並發起 PUT Commit 原地覆蓋
      const base64Content = Utilities.base64Encode(Utilities.newBlob(htmlContent).getBytes());
      const putPayload = {
        message: `Update report via Web Admin: ${fileName}`,
      ...
      if (putResp.getResponseCode() === 200 || putResp.getResponseCode() === 201) {
        return jsonResponse({ success: true, message: "GitHub File Updated via Commit" });
      } else {
        return jsonResponse({ success: false, error: "GitHub API 覆蓋失敗: " + putResp.getContentText() });
      }
    }
    ...
    if (action === "save_report") {
      ...
      const categories = Array.isArray(body.categories) ? body.categories.join(",") : (body.categories || "KYC");
